...
export default function DesignUploader({
    ...
    // Tiling logic: If disabled, force the tile size to match the image size (effectively no repeat)
    ...
    // Stagger usually means shifting every other row, but standard SVG <pattern> doesn't support this natively easily.
    // We will map "Stagger" to a SkewX for a dynamic effect, or just a simple additional offset.
    // Given the request "35.6", it seems like a specific visual shift. Let's use it as SkewX.
                ...
                {!maskDataUrl ? (
                    ...
                    <div className="relative flex justify-center items-center py-10 min-h-[500px]">
                        ...
                        <svg ref={svgRef} viewBox={`0 0 ${imageSize?.width} ${imageSize?.height}`} className="w-full h-full max-h-[600px] object-contain" xmlns="http://www.w3.org/2000/svg">
                            ...
                            {/* Apply dilated mask if offset > 0 */}
                            <mask id="finalMask">
                                <image
                                    href={maskDataUrl}
                                    width={imageSize?.width}
                            ...
                            <rect width={imageSize?.width} height={imageSize?.height} fill="url(#engravePattern)" mask="url(#finalMask)" />
                        </svg>
                    </div>
                )}
