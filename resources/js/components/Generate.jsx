...
export default function Generate({ className, onGenerate }) {
    ...
    // 🔹 Active selections
    const [intricacy, setIntricacy] = useState(patternIntricacyOptions[0]);
    const [direction, setDirection] = useState(patternDirectionOptions[0]);
    ...
    return (
        <div className={`relative flex flex-col justify-between  bg-[#171616] rounded-[30px] px-4 pt-3 pb-5 mx-auto   max-w-[350px] lg:max-w-[307px]  ${className}`}>
            ...
            {activeTab === "customize" && (
                <PrimaryButton text="Generate" className="w-full" onClick={onGenerate} />
            )}
        </div>
    );
}
